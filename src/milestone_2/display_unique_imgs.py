...
for index, row in unique_images.iterrows():
    ...
    # Add the number of duplicates on the top right corner of the image
    text = f"(Color,Sat,Brig) [Duplicates]: {dominant_clr, sat_level, brightness_level} [{num_duplicates}]"
    ...
    cv2.rectangle(img, (rect_x-rect_h, rect_y), (rect_x, rect_y + rect_h), (b,g,r), -1)
    cv2.rectangle(img, (rect_x-rect_h, rect_y), (rect_x, rect_y + rect_h), (0,0,0), 3)
